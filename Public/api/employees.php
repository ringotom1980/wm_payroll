...
function scalar(PDO $pdo, string $sql, array $params = []) {
  $row = fetch_one($pdo, $sql, $params); if (!$row) return null; return reset($row);
}
function like_q(string $q): string { return '%' . str_replace(['%', '_'], ['\\%','\\_'], $q) . '%'; }
function in_today_or($dateStr, $fallback): string { return $dateStr ?: $fallback; }
...
$baseCols = "emp_id, emp_no, full_name, phone, phone_mobile, address, email, hire_date, resign_date, status, dependents_count, expected_return_date";
...
if ($action === 'create') {
  ...
  if ($full_name === '' || $hire_date === '') j(false, '請完整填寫必填欄位', ['invalid_fields' => ['full_name' => ($full_name===''?'必填':''), 'hire_date' => ($hire_date===''?'必填':'')]]);
  ...
  if ($phone_mobile !== '' && scalar($pdo, "SELECT 1 FROM employees WHERE is_deleted=0 AND phone_mobile=:v LIMIT 1", [':v' => $phone_mobile])) $dupes['phone_mobile'] = '已有相同手機';
  if ($email !== '' && scalar($pdo, "SELECT 1 FROM employees WHERE is_deleted=0 AND email=:v LIMIT 1", [':v' => $email])) $dupes['email'] = '已有相同 Email';
  if ($dupes) j(false, '欄位重複，無法新增', ['invalid_fields' => $dupes]);

  $sql = "INSERT INTO employees (emp_no, full_name, phone, phone_mobile, address, email, hire_date, resign_date, status, bank_name, bank_account, dependents_count, expected_return_date, is_deleted, created_at, updated_at)
          VALUES ('', :full_name, :phone, :phone_mobile, :address, :email, :hire_date, :resign_date, :status, NULL, NULL, :dependents_count, :expected_return_date, 0, NOW(), NOW())";
  $st = $pdo->prepare($sql);
  $st->execute([
    ':full_name' => $full_name,
    ':phone' => $phone,
...
if ($action === 'update') {
  ...
  // 編修只警示不阻擋（若你想回傳 warnings 可加）
  $sql = "UPDATE employees SET
            full_name=:full_name, phone=:phone, phone_mobile=:phone_mobile, address=:address, email=:email,
            hire_date=:hire_date, resign_date=:resign_date, status=:status,
            dependents_count=:dependents_count, expected_return_date=:expected_return_date,
            updated_at=NOW()
...
if ($action === 'print') {
  ...
  $params = [];
  if ($q !== '') { $where .= " AND (full_name LIKE :q OR phone LIKE :q OR phone_mobile LIKE :q OR email LIKE :q OR address LIKE :q)"; $params[':q'] = like_q($q); }
  ...
  <!DOCTYPE html><html lang="zh-Hant"><head>
    <meta charset="utf-8"><title>列印｜員工主檔</title>
