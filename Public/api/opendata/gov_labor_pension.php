...
$url = 'https://apiservice.mol.gov.tw/OdService/download/A17000000J-020031-8So';
...
try {
  ...
  if ($mode === 'refresh' || $mode === 'refresh_tmp') {
    ...
    if ($mode === 'refresh') {
      ...
      $sql = "INSERT INTO {$tmpTable} (level_no, wage_min, wage_max, base_amount, effective_date)
              VALUES (:level_no, :wage_min, :wage_max, :base_amount, :effective_date)";
      $stmt = $pdo->prepare($sql);
    }
