...
(() => {
  ...
  if (els.mask) els.mask.addEventListener('click', ()=>{ els.mask.hidden=true; els.modal.hidden=true; });
  document.addEventListener('keydown', (e)=>{ if(e.key==='Escape' && !els.modal.hidden){ els.mask.hidden=true; els.modal.hidden=true; } });
  ...
  async function apiFetch(url,opt={}){ const r=await fetch(url,{credentials:'same-origin',...opt}); if(!r.ok) throw new Error(`HTTP ${r.status}`); return r.headers.get('Content-Type')?.includes('text/html')? r.text(): r.json(); }
  ...
  function updateFormLocks(){
    ...
    if (!editing){ els.btnCreate.disabled=false; els.btnSave.disabled=true; els.btnCancel.hidden=true; }
    else { els.btnCreate.disabled=true; els.btnSave.disabled=false; els.btnCancel.hidden=false; }
  }
  ...
  function validateRequired(){
    ...
    const reqs = ['full_name','birth_date','phone_mobile','address','emergency_contact_name','emergency_contact_mobile','dependents_count','hire_date','status'];
    reqs.forEach(k=>{ if(!f[k] && f[k]!==0){ setErr(k,'必填'); ok=false; } });
    ['birth_date','hire_date','expected_return_date','resign_date'].forEach(k=>{ if(f[k] && f[k] > today){ setErr(k,'不可晚於今天'); ok=false; } });
    if (Number(f.dependents_count) < 0){ setErr('dependents_count','必須≥0'); ok=false; }
    // 狀態聯動清理
  ...
  els.btnCreate.addEventListener('click', async ()=>{
    ...
    if (!okGo) return;
    const noDupe = await checkDupe(); if(!noDupe) return;
    const fd = toFormData({ action:'create', ...readForm() });
    const res = await apiFetch(API, { method:'POST', body: fd });
    if (!res.ok){ alert(res.message||'新增失敗'); return; }
    resetForm(); await Promise.all([loadActive(), loadResigned()]);
  });

  els.btnSave.addEventListener('click', async ()=>{
    const f = readForm(); if(!f.emp_id) return;
    if (!validateRequired()) return;
    const okGo = await confirmDialog('儲存修改','確認儲存對此員工的修改？'); if(!okGo) return;
  ...
  // 編輯模式切換
  els.btnActiveEdit.addEventListener('click', ()=>{ editModeActive=!editModeActive; $('#tblActive .op-col').hidden=!editModeActive; loadActive(); });
  els.btnResignedEdit.addEventListener('click', ()=>{ editModeResigned=!editModeResigned; $('#tblResigned .op-col').hidden=!editModeResigned; loadResigned(); });
  ...
  els.tbodyActive.addEventListener('click', async (e)=>{
    ...
    await doDelete(btn.getAttribute('data-id')); await loadActive();
  });
  els.tbodyResigned.addEventListener('click', async (e)=>{
    const btn = e.target.closest('.btnDel'); if(!btn) return;
    await doDelete(btn.getAttribute('data-id')); await loadResigned();
  });
